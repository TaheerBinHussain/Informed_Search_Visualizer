...
class PathfindingAgent:
    def __init__(self, win):
        self.win = win
        self.win.title("Dynamic Pathfinding Agent")
    ...
    def make_gui(self):
        ...
        tk.Button(mid_frame, text="Random Map (30%)", command=self.make_random_map).pack(side=tk.LEFT, padx=5)
        tk.Button(mid_frame, text="START", command=self.start_agent).pack(side=tk.LEFT, padx=5)
        tk.Button(mid_frame, text="Clear", command=self.clear_board).pack(side=tk.LEFT, padx=5)
    ...
    def draw_boxes(self):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                color = "white"
                if (r, c) == self.agent_pos: color = "green"
                elif (r, c) == self.goal: color = "red"
                elif (r, c) in self.walls: color = "black"
    ...
    def find_path(self, start_node):
        ...
        while pq:
            ...
            if curr != start_node and curr != self.goal:
                self.canvas.itemconfig(self.rects[curr], fill="lightblue")
            ...
            for n in self.get_neighbors(curr):
                ...
                if n not in visited_costs or new_g < visited_costs[n]:
                    ...
                    if n != self.goal:
                        self.canvas.itemconfig(self.rects[n], fill="yellow")
    ...
    def move_agent(self):
        ...
        if random.random() < 0.05:
            ...
            if empty_spots:
                ...
                self.canvas.itemconfig(self.rects[new_wall], fill="black")
